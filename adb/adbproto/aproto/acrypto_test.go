package aproto

import (
	"bytes"
	"crypto"
	"crypto/rsa"
	"crypto/sha256"
	"testing"
)

// https://cs.android.com/android/platform/superproject/main/+/main:system/core/libcrypto_utils/tests/android_pubkey_test.cpp;drc=b62146dcabedcacf99349793c1947797a173f0e6

var testDigest = [sha256.Size]byte{
	0x31, 0x5f, 0x5b, 0xdb, 0x76, 0xd0, 0x78, 0xc4, 0x3b, 0x8a, 0xc0,
	0x06, 0x4e, 0x4a, 0x01, 0x64, 0x61, 0x2b, 0x1f, 0xce, 0x77, 0xc8,
	0x69, 0x34, 0x5b, 0xfc, 0x94, 0xc7, 0x58, 0x94, 0xed, 0xd3,
}

var testKey = [PublicKeyEncodedSize]byte{
	,
}

var testSig = [PublicKeyModulusSize]byte{
	,
}

func TestPublicKey(t *testing.T) {
	var k PublicKey
	if err := k.UnmarshalBinary(testKey[:]); err != nil {
		t.Fatalf("key decode failed: %v", err)
	}
	t.Run("Verify", func(t *testing.T) {
		if err := rsa.VerifyPKCS1v15(GoPublicKey(&k), crypto.SHA256, testDigest[:], testSig[:]); err != nil {
			t.Fatalf("signature verification failed: %v", err)
		}
	})
	t.Run("Encode", func(t *testing.T) {
		buf, err := k.AppendBinary(nil)
		if err != nil {
			t.Fatalf("key encode failed: %v", err)
		}
		if !bytes.Equal(buf, testKey[:]) {
			t.Fatalf("key encode round-trip mismatch")
		}
		kk, err := NewPublicKey(GoPublicKey(&k))
		if err != nil {
			t.Fatalf("key go rsa round-trip failed: %v", err)
		}
		buf, err = kk.AppendBinary(nil)
		if err != nil {
			t.Fatalf("key go rsa encode failed: %v", err)
		}
		if !bytes.Equal(buf, testKey[:]) {
			t.Fatalf("key go rsa encode round-trip mismatch")
		}
	})
}
